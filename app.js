...
form.onsubmit = function (e) {
  ...
  let vacancy = (parseFloat((vacancyInput/100).toFixed(2)) * monthlyIncome).toFixed(2);
  let maint = (parseFloat((maintInput/100).toFixed(2)) * monthlyIncome).toFixed(2);
  let capex = (parseFloat((capexInput/100).toFixed(2)) * monthlyIncome).toFixed(2);
  let mgmt = (parseFloat((mgmtInput/100).toFixed(2)) * monthlyIncome).toFixed(2);
  ...
  console.log(parseFloat(tax/12));
  console.log(`mortgage payment: ${mortagePayment}`);

  console.log(`monthlyExpenses: ${monthlyExpenses}`);
  
  let netCashFlow = cashFlow(monthlyIncome, monthlyExpenses).toFixed(2);
  console.log(`acqisition costs: ${(acquisitionCosts)}`);

  let yearlyCashFlow = netCashFlow * 12;

  let cocr = cashOnCash(yearlyCashFlow, acquisitionCosts);
